...
export function buildBody(spec) {
  ...
  add(mkPart('chassis', L.chassis, 'body', chassis, new THREE.Vector3(0, 0, 0), 0));
  ...
  add(mkPart('grip', L.grip, 'body', grip, new THREE.Vector3(-1, 0, 0.12), 0.45));
  ...
  add(mkPart('top-plate', L['top-plate'], 'body', top, new THREE.Vector3(0, 1, 0), 0.6));
  ...
  evf.add(mesh(cyl(0.06 * sr, 0.06 * sr, 0.02 * sz, 32), M.glassFront, 0, 0.02 * sy, -0.12 * sz, Math.PI / 2));
  add(mkPart('evf', L.evf, 'body', evf, new THREE.Vector3(0, 0.55, -0.85), 0.95));

  // ---------- 热靴 ----------
  const shoe = new THREE.Group();
  shoe.position.set(0, 0.82 * sy, -0.05 * sz);
  shoe.add(mesh(box(0.2 * sx, 0.025 * sy, 0.18 * sz, M.metalDark)));
  shoe.add(mesh(box(0.16 * sx, 0.015 * sy, 0.14 * sz, M.darkPlastic), 0, 0.02 * sy, 0));
  add(mkPart('hot-shoe', L['hot-shoe'], 'body', shoe, new THREE.Vector3(0, 1, 0), 1.25));
  ...
  add(mkPart('mode-dial', L['mode-dial'], 'body', modeDial, new THREE.Vector3(0.2, 1, 0), 0.95));
  ...
  add(mkPart('controls', L.controls, 'body', ctrl, new THREE.Vector3(-0.25, 1, 0.1), 1.1));
  ...
  add(mkPart('mount', L.mount, 'body', mount, new THREE.Vector3(0, 0, 1), 0.6));

  // ---------- 快门帘幕 / 传感器保护帘（位置相同） ----------
  if (spec.shutter === 'shield') {
    // 无机械快门机型：传感器前方一张薄板保护帘
    const shield = new THREE.Group();
    shield.position.set(0, LENS_Y, hd - 0.07);
    shield.add(mesh(box(0.46 * sr * sk, 0.34 * sr * sk, 0.008, M.metalDark)));
    add(mkPart('sensor-shield', L['sensor-shield'], 'body', shield, new THREE.Vector3(0, 0, -1), 0.75));
  } else {
    ...
    add(mkPart('shutter', L.shutter, 'body', shutter, new THREE.Vector3(0, 0, -1), 0.75));
  }
  ...
  add(mkPart('sensor', L.sensor, 'body', sensor, new THREE.Vector3(0, 0, -1), 1.15));
  ...
  add(mkPart('ibis', L.ibis, 'body', ibis, new THREE.Vector3(0, 0, -1), 0.95));
  ...
  add(mkPart('mainboard', L.mainboard, 'body', board, new THREE.Vector3(0, 0, -1), 1.45));
  ...
  add(mkPart('lcd', L.lcd, 'body', lcd, new THREE.Vector3(0, 0, -1), 1.85));
  ...
  add(mkPart('battery', L.battery, 'body', battery, new THREE.Vector3(0, -1, 0), spec.verticalGrip ? 1.1 : 0.75));
  ...
  add(mkPart('card-slots', L['card-slots'], 'body', cards, new THREE.Vector3(-1, 0.1, 0), 0.55));

  // ---------- 一体式竖拍手柄（仅 verticalGrip 机型） ----------
  if (spec.verticalGrip) {
    ...
    add(mkPart('vertical-grip', L['vertical-grip'], 'body', vg, new THREE.Vector3(0, -1, 0), 0.5));
  }
